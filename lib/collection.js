...
Collection.prototype.update = function (selector, doc, options) {
    ...
    this._coll()
        .then(function (coll) {
            return Q.ninvoke(coll, 'update', selector, {
                $set: doc
            }, options);
        })
        .then(deferred.resolve)
...
Collection.prototype.updateUpsert = function (selector, doc) {
    return this.update(selector, doc, {
        upsert: true,
        w: 1
    });
};
...
Collection.prototype.findAndModify = function (query, sort, doc, options) {
    ...
    this._coll()
        .then(function (coll) {
            return Q.ninvoke(coll, 'findAndModify', query, sort, {
                $set: doc
            }, options);
        })
        .then(deferred.resolve)
...
Collection.prototype.findAndModifyOrUpsert = function (query, sort, doc) {
    return this.findAndModify(query, sort, doc, {
        new: true,
        upsert: true,
        w: 1
    });
...
Collection.prototype.find = function (query, options, sort, skip, limit) {
    var deferred, cursor;
    deferred = Q.defer();

    this._coll()
        .then(function (coll) {
            cursor = coll.find(query || {}, options || {}).sort(sort || {
                '_id': -1
